...
def kw08():
    """ lista nazwisk uczniów i nazwy klas """
    query = (Uczen
...
def kw12():
    ...
    query = (Ocena
        .select(Ocena.uczen.nazwisko, fn.AVG(Ocena.ocena).alias('sr'))
        .join(Uczen)
        .group_by(Ocena.uczen.nazwisko)
        .order_by(AVG('sr').asc())
    )
...
def kw13():
    """ średnie ocen ucznia Szymczak z poszczególnych przedmiotów """
        .select(Ocena.uczen.nazwisko, Ocena.przedmiot.przedmiot, fn.AVG(Ocena.ocena).alias('sr'))
        .join(Uczen)
